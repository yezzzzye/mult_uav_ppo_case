...
class Viewer:
    def __init__(self, width, height,caption,display=None):
        ...
        self.window = pyglet.window.Window(width=width, height=height, caption=caption,display=display,)
    ...
    def add_signal(self, signal):
        self.text_signal.append(signal)
...
class TextLine(Geom):
    ...
    def set_text(self, text, size=20,*args):
        # if pyglet.font.have_font('Secret Code'):
        #     font = "Secret Code"
        # elif pyglet.font.have_font('Courier'):
        #     font = "Courier"
        # else:
        #     return
        if pyglet.font.have_font('Times New Roman'):
...
#画扇形
def make_sector(radius=10, ang=60, filled=True):
    points = [(0,0)]
    Ang = ang/(180/math.pi)##转化为弧度制
    for i in np.arange(0,Ang,0.005):
        points.append((math.cos(i) * radius, math.sin(i) * radius))
    if filled:
...
def make_Image(fname, width, height):
