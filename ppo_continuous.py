...
class Actor_Beta(nn.Module):
    def __init__(self, args):
        ...
        if args.use_orthogonal_init:
            print("------Actor_Beta_use_orthogonal_init------")
...
class Actor_Gaussian(nn.Module):
    def __init__(self, args):
        ...
        if args.use_orthogonal_init:
            print("------Actor_Gaussian_use_orthogonal_init------")
            orthogonal_init(self.fc1)
            orthogonal_init(self.fc2)
...
class PPO_continuous():
    ...
    def save(self,agent_id,total_num_steps):
        # 创建save文件夹
        save_dir = str(self.save_dir) + "/" + str(self.date)+"/model/" + str(int(total_num_steps // self.max_episode_steps))
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
        policy_actor = self.actor
        torch.save(policy_actor.state_dict(), str(save_dir) + "/actor_agent" + str(agent_id) + ".pt")
        policy_critic = self.critic
        torch.save(policy_critic.state_dict(), str(save_dir) + "/critic_agent" + str(agent_id) + ".pt")
        # if self._use_valuenorm:
        #     policy_vnrom = self.value_normalizer
        #     torch.save(policy_vnrom.state_dict(), str(self.save_dir) + "/" + str(save_dir) + "/vnrom_agent" + str(agent_id) + ".pt")
    ...
    def restore(self,agent_id):
        ...
        self.critic.load_state_dict(policy_critic_state_dict)
        # if self._use_valuenorm:
        #     policy_vnrom_state_dict = torch.load(str(self.model_dir) + '/vnrom_agent' + str(agent_id) + '.pt')
        #     self.value_normalizer.load_state_dict(policy_vnrom_state_dict)
